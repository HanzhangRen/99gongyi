...
然而，如果我们转而考虑每一个医生个人的直接社会效益，思考这一个人对已经有277万名医生的社会产生的影响，**那么我们研究的应该是医生的边际社会效益而不是平均社会效益。**图二援引了刚刚提到的世界银行有关医生数量的数据以及华盛顿大学卫生计量与评估研究所（GHDx，2022）有关各国伤病负担的数据，试图理清世界各地医生密度与伤病负担的关系。伤病负担可以用伤残调整生命年来表述，包含了医疗服务对预期寿命的延长作用，也根据人们的主观判断折算进了伤残症状对人生活质量的影响。根据最近十年的最新数据，总体而言，似乎医生密度越大的地区伤病负担越低，但对于像中国这样医生密度已经大到一定程度的地区，两个变量的相关性就弱了许多。已有的医生已经解决了大部分可以治疗的严重伤病，而留给新增医生的发挥空间就不太多了。
...
reduce5000 <- function(data) {
  ...
  # What is the p value?
  c(exp(-.1 / glm$coefficients[[2, 1]]) - 1, glm$coefficients[[2, 4]])
}
reduce5000_output <- reduce5000(doctors)
reduce5000_output[[2]]

# All countries with doctor density above 1
doctors1 <- filter(doctors, doctors > 1.5)
reduce5000_output1 <- reduce5000(doctors1)
reduce5000_output1[[2]]
```

\doublespacing

如果我们为数据拟合一条对数函数的曲线, 可以发现，如果两组典型地区的平均人均伤病负担每年相差了十分之一个伤残调整生命年，那么负担较小的地区的医生密度中位数要比负担较大地区的医生密度中位数高上大约`r round(reduce5000_output[[1]], 3)`倍（p \< 0.001）。两组地区的伤病负担越小，同样的负担差距所对应的医生密度差距就越大。中国的医生密度达到了每千人1.980名医生。**如果我们只看医生密度超过每千人1.5名医生的地区，我们甚至可以发现在这些国家中，医生密度越大的地区伤病负担越高。**如果两组典型地区的平均人均伤病负担每年相差了十分之一个伤残调整生命年，那么负担较小的地区的医生密度中位数要比负担较大地区的医生密度中位数低大约`r round(-100 * reduce5000_output1[[1]], 1)`%（p \< 0.001）。

这绝对不是说医生是一个危害社会的职业。伤病负担的提高可能恰恰体现了医生对人类寿命的提高作用，而且我们使用的观察性数据并不支持因果推断。国家经济水平的发展可能同时提高了医生密度与寿命延长带来的疾病负担，这样一来医生密度便不一定会导致疾病负担的提高。但我们应该认识到的是，对于医生密度较高的国家，每个医生的边际社会效益是有限的，并不能让医生密度较高的地区产生较低的伤病负担。**真正减轻伤病负担的是数百万医生组成的医疗系统，而不是任何一个单独的医生。**此外，如果一个高考考生决定不当医生，医学院大概率并不会因此而减招一人，而是会降低分数线，招收另外一位考生，所以医生总数并不会因此改变。总之，一个医生职业生涯的直接社会效益是非常有限的。医生如此，那么很多其他的职业或许更是这样。
